...
namespace MasterDevs.ChromeDevTools
{
    public class ChromeSession : IChromeSession
    {
        ...
        public void Dispose()
        {
            if (null == _clientWebSocket) return;
            if (_clientWebSocket.State == WebSocketState.Open)
            {
                _clientWebSocket.CloseAsync(WebSocketCloseStatus.NormalClosure, String.Empty, new CancellationToken()).Wait();
            }
        ...
        private async Task Init()
        {
            ...
            await _clientWebSocket.ConnectAsync(new Uri(_endpoint), CancellationToken.None);
        ...
        private async Task ManageSocket()
        {
            while (_clientWebSocket.State == WebSocketState.Open)
            {
                ...
                {
                    WebSocket_MessageReceived(buffer.Array);
                    Console.WriteLine($"Received message: {Encoding.UTF8.GetString(buffer.Array, 0, result.Count)}");
                }
            ...
            //_clientWebSocket.EnableAutoPing = false;
            //_clientWebSocket.Opened += WebSocket_Opened;
            //_clientWebSocket.MessageReceived += WebSocket_MessageReceived;
            //_clientWebSocket.Error += WebSocket_Error;
        }
        ...
        private void ExecuteHandler<T>(Action<T> handler, object evnt)
        {
            if (evnt is Event<T> typedEvent)
            {
                handler(typedEvent.Params);
            }
            else
            {
                handler((T)evnt);
        ...
        private async Task<ICommandResponse> SendCommand(Command command, CancellationToken cancellationToken)
        {
            ...
            EnsureInit();
            await _clientWebSocket.SendAsync(requestString);
        ...
        private void WebSocket_Error(Exception e)
        {
            throw e;
        }
        ...
        private void WebSocket_MessageReceived(byte[] data)
        {
            ...
            throw new Exception("Don't know what to do with response: " + data);
        }
    ...
    public static class WebSocketTextSenderHelper
    {
        public static async Task SendAsync(this ClientWebSocket client, string message)
        {
